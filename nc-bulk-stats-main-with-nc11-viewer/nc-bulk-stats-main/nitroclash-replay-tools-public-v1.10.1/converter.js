...
function renderFrame(canvas, time, cameraMode, showScoreboard, showEvents, zoom = 1) {
  ...
  // Draw every name first, then every character. A player crossing another
  // player's nameplate therefore remains visible, matching NitroClash.
  for (let i = 0; i < state.players.length; i++) drawPlayerName(ctx, sx, sy, scale, state.players[i], nameAt(i, tick));
  for (let i = 0; i < state.players.length; i++) drawPlayerBody(ctx, sx, sy, scale, state.players[i], i);
...
function drawPlayerBody(ctx, sx, sy, scale, player, slot) {
...
function drawPlayerName(ctx, sx, sy, scale, player, name) {
  if (!name || !Number.isFinite(player.x) || !Number.isFinite(player.y)) return;
  const x = sx(player.x), y = sy(player.y);
  const size = 2 * PLAYER_RADIUS * scale;
