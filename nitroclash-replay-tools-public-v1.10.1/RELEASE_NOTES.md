# NitroClash Replay Tools v1.10.1

- Draws every player character above every nameplate in the replay, clip preview and exported MP4.
- Keeps names readable normally while ensuring a nearby player is never hidden underneath somebody else's name.
- Changes only the order of existing canvas drawing operations and adds no new rendering work or FPS cost.
